...
def filter_songs_data(songs_data: pd.DataFrame, track_ids: list, save_df_path: str) -> pd.DataFrame:
    ...
    """
    # filter data based on track_ids and make an explicit copy
    filtered_data = songs_data.loc[songs_data["track_id"].isin(track_ids)].copy()
    # sort the data by track id and reset index
